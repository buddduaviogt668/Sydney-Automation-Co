...
SWEEP_3_SUBURBS = [
    "Balmain", "Newtown", "Glebe", "Leichhardt", "Marrickville", "Haberfield", "Drummoyne", "Strathfield", "Concord", "Ashfield",
    "Palm Beach", "Avalon", "Newport", "Mona Vale", "Collaroy", "Freshwater", "Curl Curl", "Queenscliff", "Balgowlah", "Seaforth",
    "Castle Hill", "Baulkham Hills", "Bella Vista", "Kellyville", "Dural", "Parramatta", "Winston Hills", "Oatlands", "Epping", "Ryde",
    "Cronulla", "Caringbah", "Miranda", "Gymea", "Sylvania", "Illawong", "Menai", "Engadine", "Oatley", "Lugarno",
    "Sydney CBD", "Pyrmont", "Haymarket", "Ultimo", "Redfern", "Waterloo", "Zetland", "Alexandria", "Mascot", "Rosebery"
]
...
VERTICALS = [
    ...
    "Hospitality Hotels and Venues",
    "Corporate Offices and Boardrooms",
    "Industrial and Warehousing",
    "Retail and High-End Showrooms"
]

HUBS = ["Sydney CBD", "North Sydney", "Parramatta", "Alexandria", "Macquarie Park"]

COMMERCIAL_FAULTS = [
    "DALI gateway communication errors",
    "Emergency lighting AFSS compliance failures",
    "Network burden and DyNet bus crashes",
    "Sensor timeouts and daylight harvesting faults",
    "Timeclock and schedule overrides"
]
...
<head>
    ...
    {{
      ...
      "hasOfferCatalog": {{
        "@type": "OfferCatalog",
        "name": "Commercial Automation Services {hub}",
        "itemListElement": [
...
<body>
  ...
  ">
    <span style="color:#f0c040;font-size:13px;font-weight:600;white-space:nowrap;">
      ⚡ Priority Commercial Support — Available Today
    </span>
  ...
  <style>
    ...
    <main>
        ...
        <div class="container">
            <div class="alert-bar">
                <p style="color: #fff; font-weight: 700; font-size: 16px;">&#9888;&#65039; Commercial Lighting Emergency in {hub}? Call Now: <span style="color: #f07020; font-size: 18px;">0422 469 739</span></p>
                <p style="color: #a8c0e0; margin-top: 8px; font-size: 14px;">Certified automation technicians. Fast SLA-backed response for {hub} businesses.</p>
            </div>
        </div>
        ...
        <section class="section">
            <div class="container">
                <h3>Frequently Asked Questions — Commercial Clients</h3>
                {faq_html}
            </div>
        </section>
        ...
        <div class="container">
            <div class="cta-block">
                <h3>Secure Your Systems in {hub} Today</h3>
                <p style="color: #a8c0e0; font-size: 16px; margin-bottom: 20px;">Don't let commercial automation faults disrupt your business or void compliance.</p>
...
<footer>
        <div class="container">
            <p>&copy; 2026 Sydney Automation Co. ABN 61 136 364 150. All rights reserved. Servicing {hub} businesses.</p>
...
def get_sweep_3_content(suburb, system, fault):
    ...
    q1 = f"Why is my {system} showing {fault} in my {suburb} property?"
...
def get_sweep_4_content(vertical, hub, fault):
    title = f"{vertical}: {fault} Solutions in {hub} | Sydney Automation Co."
    description = f"Expert B2B support for {vertical} dealing with {fault} in {hub}. SLA-backed commercial automation services. Call 0422 469 739."
    h1 = f"Commercial Solutions for {fault} in {hub}"
    ...
    body_html = f"""<h3>Enterprise-Grade Automation Support for {vertical}</h3>
                <p>In {hub}, commercial properties and facility managers rely on stable lighting control systems. When you experience {fault}, generic electricians often lack the diagnostic tools required to bring the system back online without business disruption.</p>
                <p>As specialists in B2B automation support, Sydney Automation Co. provides rapid-response fault finding tailored for {vertical}. We understand the compliance and operational urgencies unique to businesses in {hub}.</p>"""
    
    q1 = f"How quickly can you address {fault} for {vertical} in {hub}?"
    a1 = f"We offer priority commercial response times for businesses in {hub}. Our technicians are fully equipped to diagnose and repair {fault} during the initial call-out."
    q2 = f"Are your services compliant with commercial regulations for {vertical}?"
...
def main():
    ...
    # SWEEP 3: 1000 Matrix Pages
    for suburb in SWEEP_3_SUBURBS:
        for system in SYSTEMS:
            for fault in FAULTS:
                slug = clean_url(f"{suburb}-{system}-{fault}")
                title, description, h1, hero_desc, body_html, faq_schema, faq_html = get_sweep_3_content(suburb, system, fault)
                
                html_content = BASE_TEMPLATE_START.format(
                    title=title, description=description, slug=slug, hub=suburb,
    ...
    # SWEEP 4: 125 Commercial Verticals
    for vertical in VERTICALS:
        for hub in HUBS:
            for fault in COMMERCIAL_FAULTS:
                slug = clean_url(f"{vertical}-{hub}-{fault}")
                title, description, h1, hero_desc, body_html, faq_schema, faq_html = get_sweep_4_content(vertical, hub, fault)
    ...
    print(f"Generated {len(generated_files)} total domination pages.")

...
def cleanup_duplicates():
    ...
    # Identify my new pages
    my_pages = [f for f in all_files if f.endswith("-cbus-dynalite-repair-sydney.html")]
    
    # Identify other pages (excluding my new ones, index, test, etc.)
    existing_pages = [f for f in all_files if f.endswith(".html") and 
                      not f.endswith("-cbus-dynalite-repair-sydney.html") and 
                      f not in ["index.html", "test.html", "google_submission_guide.html"]]
    
    removed_count = 0
    for my_page in my_pages:
        # Extract suburb name from my page: "vaucluse-cbus-dynalite-repair-sydney.html" -> "vaucluse"
        suburb = my_page.replace("-cbus-dynalite-repair-sydney.html", "")

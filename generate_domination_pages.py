...
SWEEP_1_SUBURBS = [
    "Woollahra", "Rose Bay", "Bellevue Hill", "Kirribilli", "Neutral Bay", 
    "Cremorne", "Balmoral", "Tamarama", "Bronte", "Clovelly", 
    "Coogee", "Randwick", "Paddington", "Surry Hills", "Hunters Hill", 
    "Lane Cove", "Northbridge", "Castlecrag", "Seaforth", "Manly"
]

BATCH_1_SUBURBS = ["Mosman", "Double Bay", "Point Piper", "Vaucluse", "Darling Point"]
ALL_SUBURBS = BATCH_1_SUBURBS + SWEEP_1_SUBURBS

SYSTEMS = ["C-Bus 5508RVF", "C-Bus 5504AMP", "Dynalite DDBC1200", "Dynalite DDMC802"]
FAULTS = [
    "Blinking or flashing LED codes",
...
SWEEP_2_QUERIES = [
    {"name": "C-Bus Specialist", "slug_suffix": "c-bus-specialist"},
    {"name": "C-Bus Repair", "slug_suffix": "c-bus-repair"},
    {"name": "Dynalite Programming", "slug_suffix": "dynalite-programming"},
    {"name": "Lighting Control", "slug_suffix": "lighting-control"},
    {"name": "Emergency Lighting", "slug_suffix": "emergency-lighting"},
]
  ...
  <style>
    ...
    <header>
        <div class="container">
            ...
            <p>Certified Specialists — Serving Greater Sydney</p>
        </div>
    </header>
    ...
    <main>
        ...
        <div class="container">
            <div class="alert-bar">
                <p style="color: #fff; font-weight: 700; font-size: 16px;">&#9888;&#65039; Lighting Emergency in {suburb}? Call Now: <span style="color: #f07020; font-size: 18px;">0422 469 739</span></p>
                <p style="color: #a8c0e0; margin-top: 8px; font-size: 14px;">Certified technicians. Same-day emergency response available.</p>
            </div>
        </div>
        ...
        <section class="section">
            <div class="container">
                <h3>Frequently Asked Questions — {suburb}</h3>
                {faq_html}
            </div>
        </section>
        ...
        <div class="container">
            <div class="cta-block">
                <h3>Ready to Fix Your System in {suburb}?</h3>
                <p style="color: #a8c0e0; font-size: 16px; margin-bottom: 20px;">Don't let faults disrupt your home or business. Lock in your service call today.</p>
                <a href="/book-service" class="cta-button">Book Service &amp; Pay Deposit &#8594;</a>
                <p class="trust-points">&#10003; Same-day emergency response &nbsp;|&nbsp; &#10003; Certified technicians &nbsp;|&nbsp; &#10003; Van-stocked for immediate repair</p>
            </div>
        </div>
    </main>
    ...
    <div class="grid-2" style="gap:32px; align-items: stretch; display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); margin-top: 24px;">
      <div class="card" style="background:#0e1f3d; padding: 24px; border-radius: 8px;">
        ...
        <ul class="check-list" style="list-style: none; padding: 0;">
          ...
          <li style="margin-bottom: 8px;">&#10003; <strong style="color:#f0f4ff;">C-Bus &amp; Dynalite Programming:</strong> $150/hr</li>
          <li style="margin-bottom: 8px;">&#10003; <strong style="color:#f0f4ff;">Re-commissioning / Integration:</strong> $150/hr</li>
...
def get_sweep_1_content(suburb, system, fault):
    title = f"{system} {fault} Repair in {suburb} | Sydney Automation Co."
    description = f"Expert repair for {system} {fault} in {suburb}, Sydney. Certified technicians, same-day fault finding, emergency response. Call 0422 469 739."
    h1 = f"{system} Repair in {suburb}"
    hero_desc = f"Experiencing {fault}? We fix complex {system} issues that regular electricians can't diagnose — with same-day response across {suburb} and Greater Sydney."
    
    body_html = f"""<h3>Targeted Solutions for {system} in {suburb}</h3>
                <p>Properties in {suburb} frequently rely on {system} for their ambience and functional lighting. When you encounter {fault}, it needs specialist attention.</p>
                <p>Sydney Automation Co. holds active accreditations and carries common spare modules in the van for every visit. We specialize in diagnosing why your {system} is showing {fault} and provide a transparent fix.</p>"""
    
    q1 = f"Why is my {system} showing {fault} in my {suburb} home?"
    a1 = f"This specific fault with the {system} often indicates a hardware failure or bus communication error. Our technicians in {suburb} can diagnose and resolve this on-site."
    q2 = f"Do you carry spare {system} modules for repairs in {suburb}?"
    a2 = f"Yes, we carry a wide range of common replacement parts, including components for the {system}, to ensure we can complete most repairs in {suburb} during the first visit."
...
def get_sweep_2_content(suburb, query_name):
    title = f"{suburb} {query_name} | Sydney Automation Co."
    description = f"Looking for a {query_name} in {suburb}? We are certified specialists providing expert diagnostics, repair, and programming. Call 0422 469 739."
    h1 = f"Expert {query_name} in {suburb}"
    hero_desc = f"Specialised {query_name} services in {suburb}. Our certified technicians provide guaranteed solutions for complex lighting automation and control systems."
    
    body_html = f"""<h3>{query_name} Services for {suburb} Properties</h3>
                <p>When it comes to advanced lighting control, regular electricians often lack the proprietary software and training to resolve deep system issues. As a leading {query_name} provider in {suburb}, we deliver unmatched expertise.</p>
                <p>Whether it's a high-end residential property or a commercial venue in {suburb}, we bring the right tools, manufacturer-backed accreditations, and on-board spare parts to ensure rapid turnaround.</p>"""
    
    q1 = f"Why choose your {query_name} service in {suburb}?"
    a1 = f"We hold direct manufacturer certifications and specialize purely in lighting automation. Our fast response to {suburb} and flat-rate diagnostic guarantee sets us apart from general contractors."
    q2 = f"How fast can you provide {query_name} support in {suburb}?"
    a2 = f"We offer same-day emergency response for critical failures in {suburb}, prioritizing locations that require immediate {query_name} intervention."
...
def main():
    ...
    # SWEEP 1: 400 Matrix Pages
    for suburb in SWEEP_1_SUBURBS:
        for system in SYSTEMS:
            for fault in FAULTS:
                slug = clean_url(f"{suburb}-{system}-{fault}")
                title, description, h1, hero_desc, body_html, faq_schema, faq_html = get_sweep_1_content(suburb, system, fault)
                
                html_content = BASE_TEMPLATE_START.format(
                    title=title, description=description, slug=slug, suburb=suburb,
    ...
    # SWEEP 2: 125 Query Pages
    for suburb in ALL_SUBURBS:
        for query in SWEEP_2_QUERIES:
            slug = clean_url(f"{suburb}-{query['slug_suffix']}")
            title, description, h1, hero_desc, body_html, faq_schema, faq_html = get_sweep_2_content(suburb, query['name'])
    ...
    print(f"Generated {len(generated_files)} domination pages.")

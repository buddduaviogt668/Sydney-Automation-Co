...
for filename, data in files_to_update.items():
    ...
    # Look for the section where we can inject local context
    # Usually there is an "<h2>Why Choose Sydney Automation Co?</h2>" or "<h2>Why Choose a Specialist"
    h2_pattern = re.compile(r'(<h2>.*?</h2>)')
    
    # We will insert the landmarks text right after the first paragraph following the H1 or first H2
    # It's safer to just find a suitable place like the first <p> after <div class="container"> inside .page
    
    if "<!-- LOCAL CONTEXT INJECTED -->" not in content:
        # We find the first <h2> and insert our local context above it
